...
(function(){
  var _listeners = [];
  var _usuariosCache = []; // Cache em memória para evitar chamadas redundantes à API
  ...
  function mount(perfil){
    ...
    // Fecho de modais ao clicar no overlay de fundo
    ['modalNovoUser', 'modalEditarUser'].forEach(function(id){
      var el = document.getElementById(id);
      if(el) _al(el, 'click', function(e){ if(e.target === el){ id === 'modalNovoUser' ? _fecharModal() : _fecharModalEditar(); } });
    });

    // Vinculação programática dos eventos de mudança de Perfil (Role)
    var selNovo = document.getElementById('userRole');
    if(selNovo) _al(selNovo, 'change', function(){ _onRoleChange('userRole', 'roleInfoNovo', 'grupoAcessosNovo'); });
    
    var selEditar = document.getElementById('editUserRole');
    if(selEditar) _al(selEditar, 'change', function(){ _onRoleChange('editUserRole', 'roleInfoEditar', 'grupoAcessosEditar'); });

    // Inicializar o estado visual dos grupos de acesso nos modais
    _onRoleChange('userRole', 'roleInfoNovo', 'grupoAcessosNovo');
    _onRoleChange('editUserRole', 'roleInfoEditar', 'grupoAcessosEditar');
  }

  function unmount(){
    _listeners.forEach(function(l){ l.target.removeEventListener(l.tipo, l.fn); });
    _listeners = []; _usuariosCache = []; spaResetHeader(); window.__admin = null;
  }
  ...
  function _carregar(){
    ...
    listarUtilizadores()
      .then(function(users){
        _usuariosCache = users; // Atualiza a cache local
        ...
        users.forEach(function(u){
          ...
          // Renderização HTML semântica e limpa para conformidade com tabelas e suporte responsive via CSS
          tr.innerHTML = 
            '<td data-label="Utilizador">' +
              '<div class="mobile-only user-card-header"><span class="user-card-nome">' + nome + '</span><span class="user-card-badges">' + bRole + bEst + '</span></div>' +
              '<div class="mobile-only user-card-email">' + email + '</div>' +
              '<div class="mobile-only user-card-ultimo-login">🕐 Último Login: ' + ul + '</div>' +
              '<div class="mobile-only user-card-actions">' + bEd + bTg + '</div>' +
              '<strong class="desktop-only">' + nome + '</strong><span class="desktop-only user-email-sub">' + email + '</span>' +
            '</td>' +
            '<td data-label="Role" class="desktop-only">' + bRole + '</td>' +
            '<td data-label="Estado" class="desktop-only">' + bEst + '</td>' +
            '<td data-label="Último Login" class="desktop-only"><span class="ultimo-login-txt">' + ul + '</span></td>' +
            '<td data-label="Ações" class="desktop-only"><div class="user-actions">' + bEd + bTg + '</div></td>';
          tb.appendChild(tr);
        });
      })
      .catch(function(err){
  ...
  function _onRoleChange(selId, infoId, grupoId){
  ...
  function abrirEditar(uid){
    // Utiliza os dados em cache em vez de disparar um novo fetch global
    var u = _usuariosCache.find(function(x){ return x.uid === uid; });
    if(!u){ mostrarToast('Utilizador não encontrado localmente.', 'erro'); return; }
  ...
  // Namespace para exposição global controlada de gatilhos na DOM
  window.__admin = {
    abrirNovo: abrirNovo,
    fecharModal: _fecharModal,
    guardar: guardar,
    abrirEditar: abrirEditar,
    fecharModalEditar: _fecharModalEditar,
    guardarEdicao: guardarEdicao,
    toggle: toggle
  };
