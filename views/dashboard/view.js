...
(function() {
  ...
  var ESTRUTURA_PADRAO = [
    {id:'visao-geral', titulo:'Visão Geral', icone:'📊', descricao:'Resumo geral de visitantes por período', graficos:[{id:'g1',titulo:'Total de Visitantes — Ano Atual',sub:'',url:'',largura:'total',altura:'md'},{id:'g2',titulo:'Visitantes por Mês',sub:'Evolução mensal',url:'',largura:'dois-tercos',altura:'md'},{id:'g3',titulo:'Top 5 Nacionalidades',sub:'Mês atual',url:'',largura:'metade',altura:'md'}]},
    {id:'nacionalidades', titulo:'Nacionalidades', icone:'🌍', descricao:'Distribuição por país de origem', graficos:[{id:'g4',titulo:'Todas as Nacionalidades',sub:'Ranking completo',url:'',largura:'total',altura:'lg'},{id:'g5',titulo:'Europa vs Resto do Mundo',sub:'',url:'',largura:'metade',altura:'md'},{id:'g6',titulo:'Países Ibéricos',sub:'Portugal e Espanha',url:'',largura:'metade',altura:'md'}]},
    {id:'locais', titulo:'Locais / Postos', icone:'📍', descricao:'Análise por local de registo', graficos:[{id:'g8',titulo:'Visitantes por Local',sub:'Comparativo',url:'',largura:'total',altura:'md'},{id:'g9',titulo:'Posto de Turismo de Monsaraz',sub:'Evolução',url:'',largura:'metade',altura:'md'},{id:'g10',titulo:'Museu do Fresco',sub:'Evolução',url:'',largura:'metade',altura:'md'}]},
    {id:'sazonalidade', titulo:'Sazonalidade', icone:'📅', descricao:'Padrões sazonais e tendências', graficos:[{id:'g12',titulo:'Distribuição Semanal',sub:'Dia da semana',url:'',largura:'metade',altura:'md'},{id:'g13',titulo:'Meses de Ponta',sub:'Comparativo histórico',url:'',largura:'metade',altura:'md'},{id:'g14',titulo:'Tendência Anual',sub:'Crescimento/declínio',url:'',largura:'total',altura:'lg'}]},
    {id:'operadores', titulo:'Operadores', icone:'🏨', descricao:'Visitas de operadores e agências', graficos:[{id:'g15',titulo:'Top Operadores',sub:'Por volume',url:'',largura:'dois-tercos',altura:'md'},{id:'g16',titulo:'Tipo de Grupo',sub:'Individual vs Grupo',url:'',largura:'metade',altura:'md'},{id:'g17',titulo:'Operadores por Mercado',sub:'',url:'',largura:'total',altura:'md'}]}
  ];
  ...
  function _guardarEdicao(){
    ...
    // Guardamos os índices em memória antes de limpar o objeto _editando em _fecharEdit
    var targetSecaoIndex = _editando.si;
    var targetGraficoIndex = _editando.gi;
    var tipoEdicao = _editando.tipo;

    if(tipoEdicao === 'secao'){
      _secoes[targetSecaoIndex].titulo = titulo;
      _secoes[targetSecaoIndex].icone = document.getElementById('editSub').value.trim() || '📊';
      _secoes[targetSecaoIndex].descricao = document.getElementById('editUrl').value.trim();
    } else {
      var obj = {
        id: targetGraficoIndex === -1 ? 'g' + Date.now() : _secoes[targetSecaoIndex].graficos[targetGraficoIndex].id,
        titulo: titulo,
        sub: document.getElementById('editSub').value.trim(),
        url: document.getElementById('editUrl').value.trim(),
        largura: document.getElementById('editLargura').value,
        altura: document.getElementById('editAltura').value
      };
      if(targetGraficoIndex === -1){
        _secoes[targetSecaoIndex].graficos.push(obj);
      } else {
        Object.assign(_secoes[targetSecaoIndex].graficos[targetGraficoIndex], obj);
      }
    }
    
    _fecharEdit();
    _guardar().then(function(){
      _render();
      _ativarTab(targetSecaoIndex);
    });
  }

  // Namespace unificado para evitar poluição global
  window.__dash = {
    editGrafico: editGrafico,
    addGrafico: addGrafico,
    editSecao: editSecao,
    delGrafico: delGrafico,
    guardarEdicao: _guardarEdicao,
    fecharEditOverlay: _fecharEdit
  };
